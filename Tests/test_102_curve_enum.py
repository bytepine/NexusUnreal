...
class TestCurveFloat:
    ...
    def test_manage_add_key(self, mcp):
        ...
        results = (r.get("results") or []) if isinstance(r, dict) else []
        assert any(e.get("success") for e in results if isinstance(e, dict)), f"add_key 无成功: {r}"
    ...
    def test_manage_set_key(self, mcp):
        ...
        results = (r.get("results") or []) if isinstance(r, dict) else []
        assert any(e.get("success") for e in results if isinstance(e, dict)), f"set_key 失败: {r}"
    ...
    def test_manage_set_interp(self, mcp):
        ...
        results = (r.get("results") or []) if isinstance(r, dict) else []
        assert any(e.get("success") for e in results if isinstance(e, dict)), f"set_interp 失败: {r}"
    ...
    def test_manage_remove_key(self, mcp):
        ...
        results = (r.get("results") or []) if isinstance(r, dict) else []
        assert any(e.get("success") for e in results if isinstance(e, dict)), f"remove_key 失败: {r}"
...
class TestCurveVector:
    ...
    def test_manage_add_key(self, mcp):
        ...
        results = (r.get("results") or []) if isinstance(r, dict) else []
        assert len([e for e in results if isinstance(e, dict) and e.get("success")]) >= 3
...
class TestUserDefinedEnum:
    def test_create(self, mcp):
        ...
        first = results[0] if isinstance(results[0], dict) else {}
        assert first.get("success"), f"创建失败: {first}"
    ...
    def test_manage_set_display_name(self, mcp):
        ...
        results = (r.get("results") or []) if isinstance(r, dict) else []
        assert any(e.get("success") for e in results if isinstance(e, dict)), f"set_display_name 失败: {r}"
    ...
    def test_manage_add_entry(self, mcp):
        ...
        results = (r.get("results") or []) if isinstance(r, dict) else []
        assert any(e.get("success") for e in results if isinstance(e, dict)), f"add_entry 失败: {r}"
    ...
    def test_manage_remove_entry(self, mcp):
        ...
        assert any(e.get("success") for e in results2 if isinstance(e, dict)), f"remove_entry 失败: {r2}"
