...
enum class EUnLuaTestEnum : uint8
{
    ...
    Value1 = 1 UMETA(DisplayName = "Value1"),
...
// UE 5.5+ 去掉 EAutomationTestFlags::ApplicationContextMask
#if ENGINE_MAJOR_VERSION > 5 || (ENGINE_MAJOR_VERSION == 5 && ENGINE_MINOR_VERSION >= 5)
#define UNLUA_TEST_APP_CONTEXT EAutomationTestFlags::EditorContext
#else
...
{
    template <typename T>
    FORCEINLINE const T& DecayArg(const T& Value)
    {
        return Value;
    }

    FORCEINLINE FString DecayArg(const char* Value)
    {
        return FString(UTF8_TO_TCHAR(Value ? Value : ""));
    }

    template <size_t N>
    FORCEINLINE FString DecayArg(const char (&Value)[N])
    {
        return FString(UTF8_TO_TCHAR(Value));
    }
}

FORCEINLINE bool UnLuaTestEqual(FAutomationTestBase& Test, const TCHAR* What, const char* Actual, const char* Expected)
{
    return Test.TestEqual(What, UnLuaTestPrivate::DecayArg(Actual), UnLuaTestPrivate::DecayArg(Expected));
}

template <typename TActual, typename TExpected>
FORCEINLINE bool UnLuaTestEqual(FAutomationTestBase& Test, const TCHAR* What, const TActual& Actual, const TExpected& Expected)
{
    return Test.TestEqual(What, UnLuaTestPrivate::DecayArg(Actual), UnLuaTestPrivate::DecayArg(Expected));
}
...
UnLuaTestEqual(*this, text, expression, expected);
